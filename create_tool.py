...
def add_layers(data_layer_dict, shape, viewer, config, load_labels_name=''):
    ...
    for layer_name in data_layer_dict.keys():
        ...
        # If not a image-type layer, process as labels
        else:
            ...
            if layer_name.upper() == load_labels_name.upper():
                editing_data = data[..., 0].astype(int)

    # Check to see if editing data was found... if not, store as zeros
    if editing_data is None:
        if load_labels_name:  # ambigous name was provided (not found)
            warnings.warn(
