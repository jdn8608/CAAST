...
VIEW_ORDER = ["491A", "290A", "000N", "290F", "491F"]
...
def get_bands(hdf_file, band_names, num_of_channels):
    ...
    for i, name in enumerate(band_names):
        band_data[..., i] = np.array(
            hdf_file['HDFEOS']['GRIDS'][name]['Data Fields']['I'])
        print(
            f"{name}:\t  min:{np.nanmin(band_data[...,i])} max:{np.nanmax(band_data[...,i])}"
        )

    band_data[band_data < -1] = -1
...
def read(files, config=None):
    ...
    files = [f for f in files if "AIRMSPI" in f.upper()]
    ...
    for i, (view, filepath) in enumerate(zip(views, files)):
        hdf_file = h5.File(filepath, 'r')

        # If bands_to_get is 'ALL', on first file pass, grab the band names
        if band_names is None:
            band_names = list(hdf_file['HDFEOS']['GRIDS'].keys())
            band_names.remove('XDim')
            band_names.remove('YDim')
            band_names.remove('Ancillary')
            band_names = np.array(band_names)

        # Get the band data
        band_data[i] = get_bands(hdf_file, band_names, num_of_channels)

        angle = np.array(hdf_file['HDFEOS']['GRIDS']['645nm_band']
                         ['Data Fields']['View_zenith'])
        angle = np.mean(angle[angle != -999])
        angles.append(angle)
    ...
    output_template = files[0].replace(views[0], '<view>') if files else ''

    return data_layer_dict, output_template, image_shape, (views, angles)
